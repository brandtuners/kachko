...
test('identity journey, persistence, ownership, CSRF, revocation and distributed limits', { timeout: 180000 }, async t => {
  ...
  const { PasswordResetMailer } = require('../../dist/modules/identity/password-reset.mailer');
  let resetToken;
  app.get(PasswordResetMailer).send = async (email, token) => { assert.equal(email, 'girish@example.com'); resetToken = token; };
  assert.equal((await req('/auth/password-reset/request', { method: 'POST', body: { email: 'missing@example.com' } })).status, 202);
  assert.equal((await req('/auth/password-reset/request', { method: 'POST', body: { email: 'girish@example.com' } })).status, 202);
  assert.match(resetToken, /^[A-Za-z0-9_-]{43}$/);
  assert.equal((await req('/auth/password-reset/confirm', { method: 'POST', body: { token: 'x'.repeat(43), password: 'new-password-456' } })).status, 400);
  assert.equal((await req('/auth/password-reset/confirm', { method: 'POST', body: { token: resetToken, password: 'new-password-456' } })).status, 200);
  assert.equal((await req('/auth/password-reset/confirm', { method: 'POST', body: { token: resetToken, password: 'another-password-789' } })).status, 400, 'reset token is single-use');
  assert.equal((await req('/auth/me', { cookie: otherCookie })).status, 401, 'password reset revokes sessions');
  assert.equal((await req('/auth/login', { method: 'POST', body: { email: 'girish@example.com', password: registration.password } })).status, 401);
  assert.equal((await req('/auth/login', { method: 'POST', body: { email: 'girish@example.com', password: 'new-password-456' } })).status, 200);
  await flushLimits();
  ...
  assert.equal(collision.body.data.linkRequired, true);
  const linkPendingCookie = extractCookie(collision, 'kachko_google_link_pending');
  assert.equal(await db.googleAccount.count(), 1, 'no automatic email linking');
  assert.equal((await req('/auth/google/link/complete', { method: 'POST', cookie: linkPendingCookie, body: {} })).status, 401);
  const existingLogin = await req('/auth/login', { method: 'POST', body: { email: registration.email, password: registration.password } });
  assert.equal(existingLogin.status, 200);
  const existingCookie = extractCookie(existingLogin, 'kachko_session');
  const linked = await req('/auth/google/link/complete', { method: 'POST', cookie: `${linkPendingCookie}; ${existingCookie}`, body: {} });
  assert.equal(linked.status, 200, JSON.stringify(linked.body));
  assert.equal(linked.body.data.linked, true);
  assert.equal(await db.googleAccount.count(), 2);
  assert.ok(await db.auditLog.findFirst({ where: { action: 'GOOGLE_ACCOUNT_LINKED', actorId: stored.id } }));
  assert.equal((await req('/auth/google/link/complete', { method: 'POST', cookie: `${linkPendingCookie}; ${existingCookie}`, body: {} })).status, 401, 'link grant is single-use');
  flow = await beginGoogle();
  const linkedLogin = await callback(flow, 'collision');
  assert.equal(linkedLogin.body.data.onboardingRequired, false);
  assert.equal(linkedLogin.body.data.user.id, stored.id);
  flow = await beginGoogle();
