...
const examples = {
  IMAGE: { url: 'https://example.com/photo.png', alt: 'Mountain' },
...
test('all additional blocks validate for creation and public output', () => {
  for (const [type, content] of Object.entries(examples)) {
    ...
    assert.equal(publicBlockSchema.parse({ id: '00000000-0000-4000-8000-000000000001', type, content }).type, type);
...
test('reject unsafe image, embed and contact inputs', () => {
  for (const [type, content] of [
    ['IMAGE', { url: 'data:image/svg+xml,bad', alt: 'bad' }],
