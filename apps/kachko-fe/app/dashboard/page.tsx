  ...
  return <>
    <div className="relative min-h-[150px] overflow-hidden">
      ...
      <div className="relative z-[2]">
        ...
        {page.isPublished ? <Link href={`/${page.user.username}${page.isPrimary ? "" : `/${page.slug}`}`} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-1.5 text-sm font-extrabold text-[#718c1b] hover:underline">kachko.app/{page.user.username}{page.isPrimary ? "" : `/${page.slug}`}<IconArrowUpRight className="h-4 w-4" /></Link> : <p className="mt-4 text-sm font-extrabold text-[#b45309]">Your page is still a draft</p>}
      </div>
    </div>
